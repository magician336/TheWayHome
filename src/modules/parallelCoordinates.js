...
function handleAxisClick(event, d) {
    ...
    if (_selectedAxisIndex === null) {
        ...
        // 【关键修复】使用正确的变量 _focusGroup 选择子元素进行移除，避免选择错误的类名导致无法移除
        if (_isFocusMode && isTouchingFocus) {
            _focusGroup.selectAll("*").transition().duration(400).style("opacity", 0).remove();
...
function renderFocusLayer() {
    ...
    segmentsEnter.merge(segments)
        ...
        .style("stroke", d => _cScale(d[_colorKey]))
        .style("stroke-width", 1.5) // 【重点】保持 1.5px
        .transition().duration(600).ease(d3.easeCubicOut)
        .style("stroke-opacity", 1); // 【重点】不透明度设为 1
...
// 【关键修复】切换模式时，确保清理选择状态，防止“卡”在选中态
function toggleObserverMode() {
    if (!_isFreeMode) return;
    _isFocusMode = !_isFocusMode;
    
    // 退出观察者模式时，重置轴选择，确保干净的状态
    _selectedAxisIndex = null;
    
    renderLines(false);
    updateAxisStyles(); // 更新样式以反映取消选择
    updateUIButtons();
}
