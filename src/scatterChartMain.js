...
const scatterScript = [
    ...
    () => {
        ScatterPlot.renderScene({
            mode: 'scatter',
            xKey: 'discount_strength',
            yKey: 'retention_days',
            colorBy: 'discount_strength',
            colorTitle: '折扣力度指数',
            xTitle: '折扣力度指数',
            yTitle: '留存天数 (天)'
        });
    ...
    () => {
        ScatterPlot.renderScene({
            mode: 'matrix',
            xKey: 'discount_frequency', 
            yKey: 'avg_discount_rate',
            colorBy: 'year', 
            colorTitle: '发售年份',
            xTitle: '打折频率 (次/年)',
            yTitle: '平均折扣 (%)'
...
function injectScatterControls() {
    ...
    if (container.select("#scatter-controls").empty()) {
        const controls = container.insert("div", ":first-child")
            ...
            .attr("class", "parallel-chart-input-group")
            .style("display", "none")
        ...
        controls.append("span").text("X轴: ").style("font-size", "12px");
        const xSel = controls.append("select").attr("class", "parallel-chart-select").attr("id", "scatter-x-select");
        xSel.selectAll("option").data(options).enter().append("option").attr("value", d=>d.k).text(d=>d.l);
        xSel.property("value", "discount_frequency");

        controls.append("span").text("Y轴: ").style("font-size", "12px").style("margin-left", "10px");
        const ySel = controls.append("select").attr("class", "parallel-chart-select").attr("id", "scatter-y-select");
        ySel.selectAll("option").data(options).enter().append("option").attr("value", d=>d.k).text(d=>d.l);
        ySel.property("value", "avg_discount_rate");

        controls.append("span").text("颜色: ").style("font-size", "12px").style("margin-left", "10px");
        const cSel = controls.append("select").attr("class", "parallel-chart-select").attr("id", "scatter-c-select");
        ...
        const update = () => {
            ...
            ScatterPlot.renderScene({
                ...
                xTitle: getLabel(xVal), 
                yTitle: getLabel(yVal),
                colorTitle: getLabel(cVal) // [新增]
            });
        };
        ...
        controls.append("button")
            .attr("class", "parallel-chart-button")
...
function injectFreeModeStep() {
    ...
    if (container.select("[data-step='4']").empty()) {
        ...
        step.append("div").attr("class", "story-card")
            .html(`
                <h3>自由探索实验室</h3>
